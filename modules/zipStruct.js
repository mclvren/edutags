...
module.exports.func = function(app, db, fs, h, path, zip, others_n, kaf_n, stud_n, pub_n) {
  app.get("/saveStructure_zip", function(req, res) {
    var html = fs.readFileSync(__dirname+"/struct/struct.html", {
      encoding: "utf8"
    });
    ...
    res
      .zip({
        files: [{
          ...
          {
            path: path.join(__dirname, "/struct/css"),
            name: "/struct/css"
          },
          {
            path: path.join(__dirname, "/struct/fonts"),
            name: "/struct/fonts"
          },
          {
            path: path.join(__dirname, "/struct/js"),
            name: "/struct/js"
          }
        ],
